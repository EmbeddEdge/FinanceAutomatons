...
column_index = 2 #Column C for January

# Load the CSV file into a DataFrame
columns = ['Transaction Date', 'Parent Category', 'Money In', 'Money Out', 'Category']
    ...
    if excel_category:
        ...
        for row in sheet.iter_rows():
            if row[0].value == excel_category:  # Assumes category names are in the first column
                row[column_index].value = abs(total_value)
                break
